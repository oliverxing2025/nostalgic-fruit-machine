<div align="center">
  ...
  <p>
    ...
    <img alt="版本：0.6.3" src="https://img.shields.io/badge/version-0.6.3-F3A712">
...
[v0.6.3 Release](https://github.com/oliverxing2025/nostalgic-fruit-machine/releases/tag/v0.6.3)。

- **当前 CREDIT 激活宝石：**当前余额首次达到门槛时激活新宝石，不累计已经
  花掉的历史 CREDIT。
- **永久收藏：**宝石激活后永久点亮；下注、清零或变成负余额都不会使其变暗。
- **旧数据安全迁移：**升级后弃用旧的累计中奖值，并根据设备曾达到的历史最高
  CREDIT 恢复永久宝石等级。
- **单步减注：**正面蓝键短按给当前图标加1，长按约0.9秒减1；如果减掉的是
  已预扣的一注，会自动退回对应单注 CREDIT。

### 升级方式

| 下载文件 | 写入地址 | 适用场景 | 已保存数据 |
| --- | ---: | --- | --- |
| `Nostalgic-Fruit-Machine-v0.6.3-app.bin` | `0x20000` | 已确认设备身份及兼容分区布局的现有设备升级 | 保留 NVS 中的 CREDIT、统计、押注和宝石状态 |
| `Nostalgic-Fruit-Machine-v0.6.3-full.bin` | `0x0` | 全新独立安装，或明确要替换现有固件布局 | 重置 NVS 和全部游戏存档 |
...
落在 `X3` 格时，只按对应图标下注数量的3倍结算，不再叠加普通倍率。程序先用
`esp_random()` 按每格权重决定目标，再精确计算跑灯步数，保证经过至少两圈
匀速运动后减速并停在预定位置。8类图标各押1注时，当前平衡参数的理论返还率
约为106.36%，单轮净赢概率约为29.27%；此数据不包含后续大小竞猜。
...
- **橙色 LUCK：**按总押注乘以 `2/3/5/8/10/20/20` 之一结算。
- **蓝色 LUCK：**连续点亮并结算正好5个相邻格；连续过程中的 LUCK 格不付奖，
  也不会再次触发 LUCK。
