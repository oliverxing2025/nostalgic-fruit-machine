...
```text
00 橙子，01 铃铛，02 BAR 6倍，03 BAR 12倍，04 苹果，05 苹果 X3，
...
落在 `X3` 格时，对应奖励再乘3。当前版本24个格子的命中权重相同。程序先用
`esp_random()` 决定目标，再精确计算跑灯步数，保证经过至少两圈匀速运动后
减速并停在预定位置。
...
- **橙色 LUCK：**按总押注乘以 `2/2/3/3/4/5/8/10` 之一结算。
- **蓝色 LUCK：**连续点亮并结算3至4个相邻格；连续过程中的 LUCK 格不付奖，
  也不会再次触发 LUCK。
- **稀有大奖：**平均约每480次转动触发一次，包括全盘奖、大三元、小三元、
  八仙和总押注50至200倍随机奖。
